...
make install                      # venv + editable install + sim extras
python3 scripts/doctor.py         # diagnoses the environment before you hit it
```

No install is needed to try it — every script puts the repository root on
`sys.path` itself, so a fresh clone with `numpy` and `scipy` runs:
...
python3 scripts/run_mission.py --area 220 --duration 460 --deny-gps 190 --deny-for 45 --live
...
# The full rescue loop: identify, triage, drop payloads, route ground teams
...
> `scripts/`, which puts *that* directory on the path rather than the repository
> root. Re-pull (every script now self-bootstraps), or `pip install -e '.[sim]'`.
> **Do not `pip install sar`** — that is an unrelated PyPI package and it will
> shadow this one; `pip uninstall -y sar` if you already did.
> Full explanation: [`docs/HOWTO_RUN.md`](docs/HOWTO_RUN.md) §1.

Which detector runs is a deployment decision, not a code change:

```bash
SAR_DETECTOR=auto|hybrid|heuristic|neural python3 scripts/run_mission.py
```
...
| `sar/ai/` | model registry, ONNX/QNN/TFLite runtime, detector backends, calibration, export |
| `sar/hardware/` | real camera capture with skew rejection, latching safety supervisor, onboard flight loop |
| `sar/rescue/` | ballistic payload solution, drop coordinator, A* ground-team routing |
| `scripts/` | doctor, mission runner, flight test, detector evaluation, radiometry, model fetch/train/export, onboard loop, teaching assets |
| `deploy/` | systemd unit, companion-computer installer, udev rules for stable device names |
...
| `docs/` | plan, run guide, AI/dataset reference, search theory, hardware bring-up, field checklist, runbook |
| `docs/assets/` | the animations embedded in `teach.md`, regenerated by `make assets` |
...
Full mapping in [`docs/PROJECT_PLAN.md`](docs/PROJECT_PLAN.md) §8. Build order,
wiring and per-step checks in [`docs/HARDWARE_BRINGUP.md`](docs/HARDWARE_BRINGUP.md);
the companion computer installs with `sudo ./deploy/install_companion.sh`.

The flight loop deliberately **does not arm the aircraft** — that is gated behind
[`docs/FIELD_TEST_CHECKLIST.md`](docs/FIELD_TEST_CHECKLIST.md). Everything up to
that line runs on real hardware: live cameras, live EKF telemetry, triage,
